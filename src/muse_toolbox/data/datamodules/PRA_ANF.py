...
from muse_toolbox.data.simulation.microphone_array import MicrophoneArray
...
class PraAnfConfig:
    ...
    # Room dimensions (can be discrete lists or {min, max} dictionaries)
    room_dims: list[Any] = field(
        default_factory=lambda: [{"min": 4.0, "max": 8.0}, {"min": 4.0, "max": 8.0}, {"min": 2.5, "max": 4.0}]
    )
...
class PraRIRsDB(BaseRIRsDB):
    ...
    def __init__(self, root_dir: str | Path, sampling_frequency: int):
        super().__init__(root_dir=root_dir)
        self.fs = sampling_frequency
        # No file scanning needed as RIRs are generated dynamically.
        log.info(f"Initialized PraRIRsDB for on-the-fly RIR generation at {self.fs} Hz.")
...
class AnfNoiseDB(BaseNoiseDB):
    ...
    BaseSourceDB instance to dynamically load single-channel noise files.
    """

    def __init__(
        self,
        root_dir: str | Path,
        noise_source_db: BaseSourceDB,
        sampling_frequency: int,
    ):
        super().__init__(root_dir=root_dir)
        self.noise_source_db: BaseSourceDB = noise_source_db
        self.fs = sampling_frequency
        log.info(
            "Initialized AnfNoiseDB to use a noise DB for on-the-fly diffuse noise generation."
        )
...
class PraAnfScenarioGeneratorDataset(BaseScenarioGenerator):
    """
    Dataset class for generating acoustic scenarios dynamically using Pyroomacoustics 
    (PRA) for RIRs and the ANF-Generator for diffuse noise.
    """
    def __init__(
        self,
        id: str,
        ...
        """
        Initializes the dynamic scenario generator.

        Args:
            id (str): Unique identifier for this dataset split.
            num_scenarios (int): Number of scenarios to generate.
            transform (STFTtransform): STFT configuration.
            ...
            pra_anf_config (PraAnfConfig): Config specific to PRA and ANF.
            rir_db (PraRIRsDB): PRA RIR generator.
            noise_db (AnfNoiseDB): ANF diffuse noise generator.
            source_dbs (dict[str, BaseSourceDB]): Clean speech databases.
            seed (int | None): Random seed.
            acc_device (torch.device): Device for accelerated generation.
        """
    ...
    def _sample_scenario_parameters(self) -> dict[str, Any]:
        ...
        Returns:
            dict[str, Any]: Sampled scenario configuration including room dimensions, 
                RT60, mic positions, source positions, etc.
        """

        split_name = self._get_split_name()

        # Sample room dimensions and RT60
        room_dims = [sample_parameter(d) for d in self.pra_anf_config.room_dims]
        rt60 = sample_parameter(self.pra_anf_config.rt60s)

        # Sample a microphone array configuration and place it in the room
        import copy
        mic_array_config = copy.deepcopy(random.choice(self.pra_anf_config.microphone_arrays))
    ...
    def _get_sources_for_scenario(self, scenario_params: dict[str, Any]) -> list[dict[str, Any]]:
        """
        Selects clean speech clips and assigns them to the sampled spatial positions.

        Args:
            scenario_params (dict[str, Any]): The sampled scenario parameters.

        Returns:
            list[dict[str, Any]]: A list of dictionaries describing each source 
                (id, file paths, and 3D spatial position).
        """
...
class PraAnfDataModule(BaseDataModule):
    """
    Lightning DataModule orchestrating the on-the-fly generation of acoustic data
    using Pyroomacoustics (RIRs) and ANF-Generator (diffuse noise).
    """
    def __init__(
        self,
        id: str,
    ...
    def _init_noise_source_dbs(self) -> None:
        ...
        for db_name, db_config in self.noise_databases_configs.items():
            ...
            if db_class and issubclass(db_class, BaseSourceDB):
                log.info(f"Found implementation for noise source database: {db_class_name}")
                
                # Add sampling_rate if the class needs it
    ...
    def _init_pra_anf_dbs(self) -> None:
        """Initializes the PRA/ANF-specific RIR and Noise database managers."""
        self.rir_db = PraRIRsDB(
            root_dir=self.data_dir / "databases",
