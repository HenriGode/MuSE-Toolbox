...
class BaseLitModel(pl.LightningModule):
    ...
    #     # if self.trainer.track_grad_norm > 0:
    #     norms = pl.pytorch.utilities.grad_norm(self.estimator_mask_speech, norm_type=2)
    #     self.log_dict(norms)
    ...
    def transfer_batch_to_device(
        ...
        """
        Override to handle custom HeterogeneousBatch object.
        """
    ...
    def _common_step(self, batch, idx, step_type) -> tuple[dict, HeterogeneousBatch]:
        processed_batch = self(batch)
        loss = processed_batch.compute_loss(self.criterion)
        # targets = self._get_targets_from_meta(batch)
        # loss = self.criterion(output["estimated_source_activity"], targets)
        self.log_dict(
        ...
        if False:  # DEBUGGING | PROTOTYPING
            debug_dir = "Playground"
            os.makedirs(debug_dir, exist_ok=True)
            debug_filepath = os.path.join(debug_dir, f"A_{idx}.wav")
    ...
    def _metric_step(
        ...
        if False:  # DEBUGGING | PROTOTYPING
            ...
            debug_dir = "Playground"
            os.makedirs(debug_dir, exist_ok=True)
            debug_filepath = os.path.join(debug_dir, f"A_{0}.png")
            plt.figure(figsize=(12, 6))
            plt.plot(target[0].cpu().numpy(), label="Target")
            plt.plot(
                torch.argmax(preds[0], dim=-1).detach().cpu().numpy(),
                label="Prediction",
                alpha=0.7,
            )
            plt.legend()
    ...
    def training_step(self, batch, idx):
        return self._common_step(batch, idx, "train")[0]

    def validation_step(self, batch: HeterogeneousBatch, idx, dataloader_idx: int = 0):
    ...
    def on_test_epoch_end(self) -> None:
        ...
        if not dataframes:
            print("No dataframes to save from muse_toolbox.metrics.")
        ...
        # save_dir = os.path.join(self.logger.experiment.dir, "test_results")
    ...
    #         metrics_config = self.metrics_test if stage == "test" else self.metrics_val

    #         metric_list = []
    #         for met_name, params in metrics_config.items():
    #             if not met_name:
    #                 continue

    #             metric_class = getattr(metrics, met_name)
    #             # Instantiate the metric, unpacking the params dictionary as keyword arguments
    #             metric_list.append(metric_class(**(params or {})))
    ...
    def compute_complexity_metrics_fn(self):
        ...
        try:
            ...
            print(
                "Warning: Could not import formatting utilities. Printing raw values."
            )
    ...
    def check_causality_fn(self, allowed_latency_s: float = 0.010):
        """
        Check whether the model is causal within the allowed latency.
        """
        self.eval()
