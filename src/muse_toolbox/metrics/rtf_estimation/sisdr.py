...
class SISDR(BaseMetric):
    ...
    def compute_sisdr(self, deg: torch.Tensor, ref: torch.Tensor) -> torch.Tensor:
        """
        Compute SISDR.
        """
        try:
    ...
    def update(
        ...
        for bidx in range(len(preds)):
            ...
            sig_results = {}
            sig_results["all"] = self._computeSISDRmetric(
            ...
            for name in self.Allnames:
                ...
                metname = f"SISDR{name}"

                out = (
                    torch.stack([r[f"SISDRo"] for r in res])
                    if isinstance(res, list)
                    else res[f"SISDRo"]
                )
                inp = (
                    torch.stack([r[f"SISDRi"] for r in res])
                    if isinstance(res, list)
                    else res[f"SISDRi"]
                )
                ...
                if delta.ndim == 0:
                    ...
                    raise ValueError("Delta SISDR has more than 1 dimension.")
    ...
    def _computeSISDRmetric(
        ...
        """
        Compute the SISDR metric for the reference signals.
        ...
        Returns:
            dict[str, torch.Tensor]: Dictionary containing SISDR metrics.
        """
        sig_results = {}

        sisdr_input = self.compute_sisdr(
            input_noisy,
            input_target,
        )  # scalar
        sisdr_output = self.compute_sisdr(
            output_target,
            input_target,
        )  # scalar

        sig_results["SISDRi"] = sisdr_input
        sig_results["SISDRo"] = sisdr_output
