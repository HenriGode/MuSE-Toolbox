...
class PESQ(BaseMetric):
    ...
    def __init__(
        self, transform: STFTtransform, mode: str, n_processes: int = 1, *args, **kwargs
    ):
    ...
    def robust_pesq(self, deg: torch.Tensor, ref: torch.Tensor) -> torch.Tensor:
        """
    ...
    def update(
        ...
        for bidx in range(len(preds)):
            ...
            sig_results = {}
            sig_results["all"] = self._computePESQmetric(
            ...
            for name in self.Allnames:
                ...
                out = (
                    torch.stack([r[f"PESQo"] for r in res])
                    if isinstance(res, list)
                    else res[f"PESQo"]
                )
                inp = (
                    torch.stack([r[f"PESQi"] for r in res])
                    if isinstance(res, list)
                    else res[f"PESQi"]
                )
                ...
                if delta.ndim == 0:
                    ...
                    raise ValueError("Delta PESQ has more than 1 dimension.")
    ...
    def _computePESQmetric(
        ...
        """
        Compute the PESQ metric for the reference signals.
        ...
        Returns:
            dict[str, torch.Tensor]: Dictionary containing PESQ metrics.
        """
        sig_results = {}

        pesq_input = self.robust_pesq(
            input_noisy,
            input_target,
        )  # scalar
        pesq_output = self.robust_pesq(
            output_target,
            input_target,
        )  # scalar

        sig_results["PESQi"] = pesq_input
        sig_results["PESQo"] = pesq_output
