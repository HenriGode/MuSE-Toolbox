...
from muse_toolbox.utils import activity_dict2tensor, STFTtransform, move2device
...
class STOI(BaseMetric):
    ...
    def __init__(self, transform: STFTtransform, extended: bool, *args, **kwargs):
    ...
    def update(
        ...
        for bidx in range(len(preds)):
            ...
            sig_results = {}
            sig_results["all"] = self._computeSTOImetric(
            ...
            for start, end in zip(seg_borders[:-1], seg_borders[1:]):
                Nseg = end - start
                # if Nseg < 7500:
                #     raise ValueError("Segment too short for STOI computation.")
                ...
                if segid in self.Segnames:
                    ...
                    #     with open("./results/stoi_error_log.txt", "a") as f:
                    #         f.write(
                    #             f"Scenario ID: {meta['scenario_id'][bidx]}, Segment ID: {segid}, Start: {start}, End: {end}\n"
                    #         )
            ...
            for name in self.Allnames:
                ...
                out = (
                    torch.stack([r[f"STOIo"] for r in res])
                    if isinstance(res, list)
                    else res[f"STOIo"]
                )
                inp = (
                    torch.stack([r[f"STOIi"] for r in res])
                    if isinstance(res, list)
                    else res[f"STOIi"]
                )
                ...
                if delta.ndim == 0:
                    ...
                    raise ValueError("Delta STOI has more than 1 dimension.")
    ...
    def _computeSTOImetric(
        ...
        """
        Compute the STOI metric for the reference signals.
        ...
        Returns:
            dict[str, torch.Tensor]: Dictionary containing STOI metrics.
        """
        sig_results = {}

        stoi_input = self.STOI_fun(
            input_noisy.squeeze(0),
            input_target.squeeze(0),
        )  # scalar
        stoi_output = self.STOI_fun(
            output_target.squeeze(0),
            input_target.squeeze(0),
        )  # scalar

        sig_results["STOIi"] = stoi_input
        sig_results["STOIo"] = stoi_output
